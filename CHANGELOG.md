...
## v2.1.0 — Snail garden progression system (built, DO NOT DEPLOY)

- **Garden runners** (`v garden pet`, alias `gardenpet`): 9 unlockable runners — snail (1.25x / 20% base fail) → frog (1.24x / 20%) → worm (1.26x / 21%) → cat (1.20x / 17% +5% step) → bunny (1.27x / 22%) → hedgehog (1.22x / 19%) → fox (1.28x / 23%) → owl (1.23x / 19%) → dragon (1.30x / 24%). Unlock levels 1/2/2/3/3/5/5/7/9. Every runner satisfies `(1 − failBase/100) × mult ≤ 1`, so the one-row sprint is exactly fair (EV 1.0) and deep rows are house-favored.
- **Garden upgrade shop** (`v gardenshop` / `v gardenbuy <upgrade>`, alias `garden shop`): soil (−1%/lvl fail-less growth bonus rows 5+, lvl 1, 250k·lvl), seeds (+1.5%/lvl profit rows 5+, lvl 1, 300k·lvl), net (4%/lvl rescues a failed row 5+ as a sell, lvl 3, 500k·lvl), boots (+1 row cap/lvl, max 3 → 13 rows, lvl 1, 200k·lvl), golden can (+2% profit rows 5+ forever, lvl 6, 5M flat). All effects are deep-gated (`GARDEN_DEEP_DEPTH`=5) so early-bet EV is never touched; every effect is either survivability-neutral or a straight deep-row profit mult with matching fail pressure.
- **XP / levels**: `rowXp = rows × (1 + floor(bet/10000)) + bonus floor(cashed/50000)`; hard cap 300 XP/day (`GARDEN_XP_DAY_CAP`, anti-farm); level curve `floor(40 × L^1.5)`; level-up coins `L × 5000`. `v garden` shows the level bar + run stats.
- **Deep-row events** (12% on rows 5+ only): Light Rain (−6% fail, +8% grow), Sunshine (+10% grow), Bugs (+8% fail), Bedrock (nothing). Rows 1–4 stay boring/fair.
- **Achievements**: `garden_first` (15k), `garden_10` (100k), `garden_100` (300k), `garden_perfect` (500k), `garden_safety` (50k), `garden_million` (750k). **Titles**: `garden_keeper` (level 5, rare), `master_gardener` (level 10, legendary). Auto-unlock via the existing `checkAchievements` hook (reads lightweight `gardenStatsFor` SELECT-only stats, no insert).
- **DB layer**: `garden_meta` (runs/wins/perfects/level/xp/cashed_best/run_streak) + `garden_unlocks` (runners/upgrades/equipped) tables; constants + helpers in `db/index.js` (`gardenRunnerStats`, `gardenMods`, `gardenRowGrowth`, `gardenMultAt`, `gardenFailAt`, `gardenEvent`, `addGardenXp`, `unlockGardenRunner`, `equipGardenRunner`, `buyGardenUpgrade`, `getGardenProfile`, `gardenStatsFor`, `recordGardenResult`).
- **`v garden` conflict resolved**: no-arg `v garden` now shows the combined profile (runner, level bar, upgrades, win/loss) while keeping `buy`/`sell`/`snails` breeding intact.
- **Balance audit + Monte Carlo** (`scripts/test-garden-balance.js`): N=30k per (runner × fresh/mid/max upgrades) × 13 strategies at factor 1.0. **No config is +EV** — best strategy for every runner is sell-at-0 (EV exactly 1.0000); dragon/max-upgrade probe shows EV declining monotonically from 1.0000. The historical fair baseline is preserved; upgrades can only shift you into more variance, never free EV.
- **Anti-farm hardening (pre-deploy audit, no economy change):** an *untouched* garden (sell or timeout before any row) is a **pure refund** — it records NO run, no XP, no stats, no achievements, and does NOT count as wagered (`gamble` counters / weekly board / battlepass XP only start on the first `Next` press, when real risk begins). Garden **XP is staked-gated** (`GARDEN_PROG_MIN_BET` = 10k — a sub-10k bet earns zero XP) and the run-count **achievements require lifetime stakes** (garden_first 10k / garden_10 100k / garden_100 1M) so enter-and-sell spam cannot farm XP, levels, level-up coins, achievements or titles at any bet size. Proven by `scripts/test-garden-anti-farm.js` (39/39 deterministic: 300 × 1-coin and 10 × 100k zero-risk sell-loops move zero counters; real runs unlock progression exactly once; lucky flag has zero effect; no garden objective in quests/bounties; battlepass XP flat under sub-gate spam).
- **Tests**: `scripts/test-garden.js` 67/67 (levels, XP/cap/rollover, runner gating, EV invariant, upgrades, deep-gating, events, achievements/titles, E2E). Full suite re-run: only the 5 pre-existing Node-26 baseline failures (test-2.0/8ball/owner-give/social-commands/social) unchanged.
