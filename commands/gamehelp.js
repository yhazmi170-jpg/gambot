  ...
  execute(message, args) {
    ...
    const games = {
      ...
      snailgarden: {
        ...
        play: `\`${prefix} sg <amount>\`\nHit **Next row** to plant, or **Sell** to cash out\nExample: \`${prefix} sg 1000\``,
        tips: 'Cash out early for a safe small win. The multiplier compounds each row (~1.25x), failure chance climbs as you go. Garden runners (v gardenpet), upgrades (v gardenshop) and levels change your planters — deep rows only, so the first row stays fair.',
      },
      gardenpet: {
        name: 'Garden Runners',
        aliases: 'gp',
        desc: 'Pick which pet plants your garden. Runners unlock by garden level.',
        play: `\`${prefix} gardenpet [runner]\`\nNo arg lists all runners.\nExample: \`${prefix} gardenpet cat\``,
        tips: 'Level up by playing v sg — bigger bets, more xp (daily cap). Safer runners grow slower but fail less.',
      },
      gardenshop: {
        name: 'Garden Shop',
        aliases: 'gshop',
        desc: 'Permanent upgrades for your garden — boosts only apply to deep rows (5+).',
        play: `\`${prefix} gardenshop\`\nBuy upgrades with ${prefix} gardenbuy <item>` + '\nExamples: soil, seeds, net, boots, can',
        tips: 'Upgrades help you survive/win the deep rows where the multiplier is huge but the risk is too. They never touch the fair first row.',
      },
      gardenbuy: {
        name: 'Garden Buy',
        aliases: 'gbuy',
        desc: 'Buy a garden upgrade from the garden shop.',
        play: `\`${prefix} gardenbuy <upgrade>\`\nExample: \`${prefix} gardenbuy net\``,
        tips: 'Check v gardenshop for prices and level requirements.',
      },
      wheel: {
        name: 'Wheel',
        aliases: 'wheel, spin',
