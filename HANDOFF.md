...
- **Branch**: `master`
- **Version**: `1.4.3`
- **Last update_msg**: `v1.4.3 -- fixed data loss on restarts: the bot now saves your progress to the cloud BEFORE shutting down, and backs up every 10 minutes instead of every hour — so updates no longer wipe recent balance/pets/essence`
...
- [ ] Confirm v1.4.3 deploy live on Render (update embed in `v version`)
- [ ] **Restore friend's lost ~30m** (`554257220523655199`, @伤害): the v1.4.2 deploy REVERTED any restore — live bal back at **2,445,528** (gems 0). Re-run `Aovo add @伤害 <diff>` AFTER v1.4.3 deploys (shutdown+10-min backups will then protect it). Target: 32,220,527 (snapshot `1802448`, 2026-08-05T22:50:48Z). Diff from 2,445,528 = **29,774,999** — recheck `v bal @伤害` first.
