...
- [x] **TOS 'I Agree' fixed (2026-09-12):** users reported "can't even agree" — the button seemed dead and the prompt kept coming back. Root cause: `agree.js` called `interaction.update()` with no catch; the global interaction ack-fallback (index.js 2.5s `deferUpdate`) could fire first whenever the event loop stalled (sql.js backup every 1 min), making `interaction.update()` throw "already acknowledged" → `db.acceptTerms()` never ran (unhandled rejection) → user stayed unregistered → TOS re-prompted every command ("same shit"). Fix: ack instantly + confirm via plain `msg.edit` (idempotent against pre-acks), `acceptTerms` wrapped in try/catch + idempotent, and `tos_accepted`/`tos_accept_failed` traced to `/debug`. `acceptTerms` re-verified against a throwaway DB (with `node /tmp/opencode/tos-test.js`). `commands/agree.js` + `utils/commandHandler.js` sendTosPrompt both hardened. Ship without version bump (small fix → pending_updates.txt).
