...
- **Current version**: see `package.json` (as of last docs sync: **1.7.1**)
...
- **Travelling merchant:** arrives every ~90 min (first 5 min after boot), posts to event/update channels; `v merchant` shows 3 one-time slots (rare pet grab-bag + gems×10 + essence×25) with buttons `mer_<slot>` (`handleInteraction` routed from index.js). DB: `merchant_state` (next_at) + `merchant_stock` (slot, kind, label, price, sold_to, extra). Pets spawn owned by `MERCHANT_OWNER_ID = '__merchant__'` and transfer to buyer via `UPDATE animals`.
