...
- **Snail garden** (`v snailgarden <amt>`, `v sg`): step-by-step planting game. Failure chance starts at 20% and increases by 7% per step (max 95%). Cash out before a row fails. **Progression (v2.1.0):** `v garden pet` picks a runner (snail/frog/worm/cat/bunny/hedgehog/fox/owl/dragon — 9, unlock levels 1/2/2/3/3/5/5/7/9, `db.GARDEN_RUNNERS`), `v gardenshop`/`v gardenbuy` buys permanent upgrades (soil/seeds/net/boots/can, `db.GARDEN_UPGRADES`), XP/levels (`db.gardenXP` anti-farm 300/day cap, level `floor(40·L^1.5)`, level-up coins `L·5000`). Balance rule: every runner satisfies `(1−failBase/100)·mult ≤ 1` and ALL beneficial effects are deep-gated (`GARDEN_DEEP_DEPTH`=5, `db.gardenMods`); deep-row events (12%: rain/sunshine/bugs/bedrock). Audited by `scripts/test-garden-balance.js`: no runner/upgrade combo is +EV. Anti-farm: a 0-row sell (or timeout) is a pure refund — no run/XP/stats/achievements and NOT wagered (`addGambled` fires on the first `sg_next` only); XP requires stake ≥ `GARDEN_PROG_MIN_BET` (10k); run-count achievements require lifetime `total_staked` gates (10k/100k/1M). Proven by `scripts/test-garden-anti-farm.js`. `v garden` = combined profile (runner/levels/upgrades) + breeding (buy/sell preserved).
...
- **Pet weapons (v1.8.0, OwO-style):** `v weaponcrate` buys (15k coins, `db.WEAPON_CRATE_PRICE`) / opens weapon crates; crates also drop ~8% (`db.WEAPON_BATTLE_DROP_CHANCE`) per animal from `v hunt` and from `v battle` wins; shop has a `Weapon Crate` item (`weaponCrates: 1` handled in shop confirm). `v weapon list` / `v weapon equip #<weaponId> #<animalId>` (animal must be on battle team) / `v weapon upgrade #<weaponId>` (coins, `db.weaponUpgradeCost`). Tables: `weapons_inv` (type/rarity/quality/level), `animals_weapon` (animal_id → weapon_id), `weapon_crates` (user qty). Rarity order common→uncommon→rare→epic→mythical→legendary→fabled (`WEAPON_RARITY_ORDER`/`WEAPON_RARITY_DATA`), quality % per rarity; weapon ATK/DEF = base × quality × level growth (`weaponAtk`/`weaponDef`). Battle integration in `commands/battle.js`: weapons add flat ATK/DEF on top of trait/fed % (aegis leans DEF), effects in the loop — great_sword splash, poison_dagger/flame_staff DoT (statuses), vamp_staff lifesteal, heal_staff heals weakest teammate, aegis taunt redirects next attacks, bow/rune passthrough. Equipped weapon shown in `v team`.
...
- **Achievements / black market (v1.5.0):** `v achievements` + auto-unlock check hooked into `utils/commandHandler.js` after every command (50 in `db.ACHIEVEMENTS`). `v blackmarket` = 4 rotating slots refreshed every 6h (`db.refreshBlackMarket`), 6 item types.
- **Animal dex (2.0):** `v dex [rarity|shiny|missing]` uses the full **92**-species catalog via `db.dexCatalog`/`db.dexProgress` + `db.DEX_MILESTONES`; per-rarity completion, shiny tracking and `db.nextDexMilestone` drive achievements/titles. `db.getOwnedSpecies` remains the raw per-user map.
- **Pet bond (2.0):** `animals.bond` + `db.BOND_TIERS` (Stranger→Soulbound) gives a passive battle multiplier (`db.bondMultiplier`, +2%..+10%); bond grows from leveling/feeding/evolving/renaming/battles (shiny 2x). Surfaced in `v animal`/`v team`.
- **Quests/bounties (2.0):** 17 objective types via `db.QUEST_OBJECTIVES` + eligibility (`db.QUEST_REQUIRES`/`db.questContext`) so impossible dailies are never rolled; rare tiers pay `seal_reward`. Hooks live in the real earning paths (hunt/hatch/dex/petlevel/bond/merchant/inbox/streak/xp/social/event/contract).
- **Community events (2.0):** original 5 + `great_hunt` (per-guild coop goal, `community_event_progress`), `gem_rush`, `pet_festival`, `dex_challenge`; coop completion grants contributors an `event_wins` (achievement `event_hero`, title `event_champion`).
- **Server lore (2.0):** `db.recordDiscoveries` logs notable finds (shiny/mythic/legendary) + `return` incidents; `v incidents`/`v lore` shows a capped rolling history.
- **Social (2.0):** `v hug/kiss/kill/slap/pat/cuddle/bite/punch/lick` send the action line as message **content** (real mention chips) with an image-only embed; `allowedMentions` pings only the target. `social_pairs` tracks actor/target/action counts and shows only first + rare milestones (10/25/50/100/250/500/1000).
- **Insurance tiers (v1.5.0, corrected 2.0):** base `insurance` = 10% loss refund; shop upgrades `insurance2/3/4` = 15/20/25% (highest owned wins, `db.getInsuranceLevel`). Plus a loss-streak bonus — see `lossStreakBonus`. Auto-applied in all games via `db.getInsuranceRefund`.
...
- Free tier spins down after **15 min idle** ? keepalive pinger must keep hitting the Render URL (`https://gambot-hle3.onrender.com`) every 4 min. It wakes on request (~30-50s). (2026-09-22: re-created on NEW Render account `rnd_8elgJ0yi9jPjEr7u8Y16J7Wu6Fdd` — old account suspended, old key dead. Sentinel also migrated: `https://sentinel-wqrw.onrender.com`.)
