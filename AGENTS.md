...
- **Balance corruption (Aug 2026)**: a bug drained multiple users' balances over time (@极极 lost ~52M, @meimei lost ~28M). Root cause unknown — may be integer overflow in balance calculations. If balances look wrong again, check `db/index.js` `addBalance`/`payWin`/`getBalanceFactor` for overflow bugs.
- **Dashboard**: Express.js web dashboard at `/web/server.js`. Discord OAuth2 login (`/auth/discord` → `/auth/callback`). Sessions stored in memory (ephemeral FS). Routes: `/` (landing), `/dashboard` (protected), `/api/me`, `/api/leaderboard`, `/api/stats`, `/api/animals`, `/auth/logout`. Env: `DISCORD_CLIENT_ID`, `DISCORD_CLIENT_SECRET`, `SESSION_SECRET`. Config: `config.discordClientId`, `config.discordClientSecret`, `config.sessionSecret`. Frontend: `web/views/` (HTML), `web/public/` (CSS/JS). Dark theme with cards, leaderboard, global stats. Auto-refreshes every 30s.
