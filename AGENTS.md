...
- **EVERY PUSH (mandatory, do not skip):** bump `package.json` version, update `update_msg.txt` with a user-facing summary of what changed, verify new commands have `helpCategory` set so they auto-appear in `v help`
